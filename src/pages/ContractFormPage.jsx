...
const ContractFormPage = () => {
  ...
  const isCustomerMode = profile?.role === 'user';
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      if (!profile?.partner_uuid || isCustomerMode) return;
    ...
    fetchAdminData();
  }, [profile, isCustomerMode]);

  // Fetch Contract for Editing
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    checkDiscountCodesAvailability();
  }, [isCustomerMode, user]);
  ...
  }, [formData.customer_id, isCustomerMode]);
  ...
  const fetchCustomerLocations = async () => {
    try {
      const { data, error } = await supabase
        .from('locations')
        .select('id, location_name, vat_percentage')
        .eq('partner_uuid', profile?.partner_uuid)
        .order('location_name');
  ...
  const checkDiscountCodesAvailability = async () => {
    try {
      const { count, error } = await supabase
        ...
        .eq('partner_uuid', profile?.partner_uuid || partnerUuid)
        .eq('is_active', true);

      if (error) {
        logger.error('Error checking discount codes:', error);
        return;
      }

      setHasDiscountCodes(count > 0);
    } catch (error) {
      logger.error('Error checking discount codes:', error);
    }
  };
  ...
  const handleConfirmContract = async () => {
    ...
    try {
      ...
      if (editMode && contractToEdit) {
        ...
        try {
          logger.log('📧 Attempting to send contract creation email...');
          const emailSent = await oneSignalEmailService.sendContractCreationEmail(result, t);
          if (emailSent) {
            logger.log('✅ Contract creation email sent successfully');
          } else {
            logger.warn('⚠️ Failed to send contract creation email (non-blocking)');
          }
        } catch (emailError) {
  ...
  const incrementDiscountUsage = async (code) => {
    try {
      const { error } = await supabase.rpc('increment_discount_usage', {
        p_partner_uuid: profile?.partner_uuid,
        p_code: code
      });
          ...
          <div className="confirmation-content">
            ...
            <div className="contract-summary">
              ...
              <div className="summary-item">
                ...
                <div className="price-breakdown-confirmation" style={{ width: '100%' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                    <span style={{ fontWeight: 'normal', color: '#6b7280' }}>{t('contracts.baseAmount') || 'Imponibile'}:</span>
                    <span>{formatCurrency(getFinalPrice(), selectedService?.currency)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #e5e7eb', paddingTop: '0.25rem', marginTop: '0.25rem', fontWeight: 600 }}>
            ...
            <div className="form-actions" style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
              ...
              <button
                ...
                className="btn-contract-modal-primary"
                disabled={loading}
              >
                {loading ? (
                  <>
